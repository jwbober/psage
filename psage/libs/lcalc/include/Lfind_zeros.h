...
//puts the result in the vector result which is passed by reference
    ...
    //
    // EDIT BY BOBER:
    //  In this version we don't care about printing zeros. We just want to compute and return them,
    //  so I'm changing this function to fill an array with zeros instead of printing them.
    find_zeros(Long count, Double * output_zeros, Long start_N, Double max_refine, int rank)
    {
        int output_index = 0;
        if(my_verbose>2)
            ...
            if(ii>=50&&!duplicates){
                ...
                do{
                    ...
                    if(count2>number_zeros_to_suppress){
                        ...
                        //
                        // CHANGED BY BOBER
                        output_zeros[output_index] = zeros_S[i][0] * zeros_S[i][2];
                        output_index++;
